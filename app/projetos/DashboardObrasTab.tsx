...
import {
  ...
  labelSituacaoObra, formatData, formatMoeda,
...
function KPICard({
  ...
  const txt = { default:"text-gray-900", green:"text-emerald-600", red:"text-red-600", amber:"text-amber-600", blue:"text-blue-600", folk:"text-folk" }[cor];
  const border = { default:"border-gray-200 bg-white", green:"border-emerald-200 bg-emerald-50", red:"border-red-200 bg-red-50", amber:"border-amber-200 bg-amber-50", blue:"border-blue-200 bg-blue-50", folk:"border-folk/20 bg-folk/5" }[cor];
  return (
...
export default function DashboardObrasTab() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      const [lista, tecs, av] = await Promise.all([
        listarObras(), listarTecnicos({ ativo: true }), listarObraAcoesVencidas().catch(() => []),
      ]);
  ...
  const acoesVencPorObra = useMemo(() => {
    ...
    acoesVenc.filter((a) => baseIds.has(a.obra_id)).forEach((a) => { m[a.obra_id] = (m[a.obra_id] ?? 0) + 1; });
    return m;
  }, [acoesVenc, baseIds]);
  ...
  const semAStr = inicioSemAtual.toISOString().slice(0,10);
  const semBStr = inicioSemAnt.toISOString().slice(0,10);
  const mesAStr = inicioMesAtual.toISOString().slice(0,10);
  const mesBStr = inicioMesAnt.toISOString().slice(0,10);

  const valSemA = useMemo(() => base.filter((o)=>["a_executar","em_execucao"].includes(o.situacao)&&o.data_inicio>=semAStr&&o.data_inicio<=HOJE).reduce((s,o)=>s+o.valor_execucao,0), [base,semAStr]);
  const valSemB = useMemo(() => base.filter((o)=>["a_executar","em_execucao"].includes(o.situacao)&&o.data_inicio>=semBStr&&o.data_inicio<semAStr).reduce((s,o)=>s+o.valor_execucao,0), [base,semAStr,semBStr]);
  const valMesA = useMemo(() => base.filter((o)=>["a_executar","em_execucao"].includes(o.situacao)&&o.data_inicio>=mesAStr&&o.data_inicio<=HOJE).reduce((s,o)=>s+o.valor_execucao,0), [base,mesAStr]);
  const valMesB = useMemo(() => base.filter((o)=>["a_executar","em_execucao"].includes(o.situacao)&&o.data_inicio>=mesBStr&&o.data_inicio<mesAStr).reduce((s,o)=>s+o.valor_execucao,0), [base,mesAStr,mesBStr]);

  const valFinSemA = useMemo(()=>base.filter((o)=>o.situacao==="finalizada"&&o.data_conclusao&&o.data_conclusao>=semAStr&&o.data_conclusao<=HOJE).reduce((s,o)=>s+o.valor_execucao,0),[base,semAStr]);
  const valFinSemB = useMemo(()=>base.filter((o)=>o.situacao==="finalizada"&&o.data_conclusao&&o.data_conclusao>=semBStr&&o.data_conclusao<semAStr).reduce((s,o)=>s+o.valor_execucao,0),[base,semAStr,semBStr]);
  const valFinMesA = useMemo(()=>base.filter((o)=>o.situacao==="finalizada"&&o.data_conclusao&&o.data_conclusao>=mesAStr&&o.data_conclusao<=HOJE).reduce((s,o)=>s+o.valor_execucao,0),[base,mesAStr]);
  const valFinMesB = useMemo(()=>base.filter((o)=>o.situacao==="finalizada"&&o.data_conclusao&&o.data_conclusao>=mesBStr&&o.data_conclusao<mesAStr).reduce((s,o)=>s+o.valor_execucao,0),[base,mesAStr,mesBStr]);

  // ── Saúde ─────────────────────────────────────────────────────

  const saude = useMemo(() => {
    if (base.length === 0) return { score: 100, label: "Excelente", cor: "green" as const };
    let pen = atrasadasFin.length*5 + paralisadas.length*10 + prazoVencido.length*3 + execucaoLonga.length*5;
  ...
  const obrasEmRisco = useMemo(() => base
    .filter((o) => {
      if (o.situacao === "finalizada") return false;
      const av = acoesVencPorObra[o.id] ?? 0;
      return (
    ...
    .map((o) => {
      const av = acoesVencPorObra[o.id] ?? 0;
      return {
        ...o, score: calcRiscoScore(o, av, hojeD), acoesV: av,
        diasAtrasoPrazo: o.data_prazo && HOJE > o.data_prazo ? diasEntre(o.data_prazo, HOJE) : 0,
        diasRestantes:   o.data_prazo ? diasEntre(HOJE, o.data_prazo) : null,
        diasExecucao:    diasEntre(o.data_inicio, HOJE),
      };
    })
  ...
  const porStatus = useMemo(() => SITUACOES_OBRA.map((s) => ({ ...s, count: filtradas.filter((o) => o.situacao === s.value).length })), [filtradas]);
  ...
  const meses = useMemo(() =>
    mesesNoPeriodo(filtros.dataInicio, filtros.dataFim).map((key) => ({
      ...
      iniciadas:  filtradas.filter((o) => dentroDeMes(o.data_inicio_previsto ?? o.data_inicio, key)).length,
      concluidas: filtradas.filter((o) => dentroDeMes(o.data_conclusao, key)).length,
      get saldo() { return this.iniciadas - this.concluidas; },
    }))
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card className="p-4">
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <button onClick={carregar} className="text-xs font-semibold text-folk hover:underline">
            ↺ Atualizar
          </button>
        </div>
      </Card>
      ...
      {carregando ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            <div className={`col-span-2 sm:col-span-1 lg:col-span-2 rounded-2xl border p-5 shadow-sm flex flex-col justify-between ${
              ...
              <div className="flex items-end gap-3">
                <p className={`text-5xl font-black leading-none ${ saude.cor === "green" ? "text-emerald-700" : saude.cor === "blue" ? "text-blue-700" : saude.cor === "amber" ? "text-amber-700" : "text-red-700" }`}>{saude.score}%</p>
                <div className="pb-1">
                  <span className={`rounded-full border px-3 py-0.5 text-xs font-bold ${ saude.cor === "green" ? "border-emerald-200 bg-white text-emerald-700" : saude.cor === "blue" ? "border-blue-200 bg-white text-blue-700" : saude.cor === "amber" ? "border-amber-200 bg-white text-amber-700" : "border-red-200 bg-white text-red-700" }`}>
                    {saude.cor === "green" ? "🟢" : saude.cor === "blue" ? "🔵" : saude.cor === "amber" ? "🟡" : "🔴"} {saude.label}
                  </span>
                </div>
              </div>
              <div className="mt-3 flex flex-col gap-1.5 text-xs border-t border-black/5 pt-3">
                ...
                {Object.keys(acoesVencPorObra).length > 0 && (
                  <p className="text-orange-600">−{Object.values(acoesVencPorObra).reduce((s,n)=>s+n*2,0)}pts ações vencidas ({Object.values(acoesVencPorObra).reduce((s,n)=>s+n,0)})</p>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <KPICard label="Valor em Execução" value={formatMoeda(valorEmExecucao)} sub="obras a executar + em execução" cor="folk"
            ...
            <KPICard label="Valor Concluído"    value={formatMoeda(valorConcluido)}  sub="obras finalizadas" cor="green"
            ...
            <KPICard label="Valor em Risco"     value={formatMoeda(valorEmRisco)}    sub="prazo vencido ou paralisadas" cor={valorEmRisco>0?"red":"default"} deltaSem={null} deltaMes={null} />
          </div>
                        ...
                        <tr key={o.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60">
                          ...
                          <td className="py-3 px-3">
                            <div className="flex flex-col gap-0.5 text-xs text-gray-600">
                              {o.diasAtrasoPrazo > 0 && <span className="text-red-600 font-semibold">{o.diasAtrasoPrazo}d de atraso no prazo</span>}
                              {o.diasRestantes !== null && o.diasRestantes >= 0 && o.diasRestantes <= 7 && <span className="text-amber-600">Prazo vence em {o.diasRestantes}d · andamento {o.andamento}%</span>}
                              {o.diasExecucao > 90 && <span className="text-orange-600">{o.diasExecucao}d em execução</span>}
                              {o.acoesV > 0 && <span className="text-rose-600">{o.acoesV} ação{o.acoesV !== 1 ? "ões" : ""} vencida{o.acoesV !== 1 ? "s" : ""}</span>}
                              {o.valor_execucao > 0 && <span className="text-gray-400">{formatMoeda(o.valor_execucao)}</span>}
                            </div>
                          </td>
                          <td className="py-3 pr-4 text-right">
                            <div className="flex flex-col items-end gap-1">
                              <span className={`rounded-full px-2.5 py-0.5 text-xs font-black ${r.cls}`}>{r.dot} {r.label}</span>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <Card className="p-6">
              ...
              {total === 0 ? <p className="text-sm text-gray-400">Nenhuma obra no período.</p> : (
                <div className="flex flex-col gap-3">
                  {porStatus.map((s) => <BarraH key={s.value} label={s.label} count={s.count} max={maxStatus} color={STATUS_COR[s.value as SituacaoObra]} />)}
                  {total > 0 && (
                    <div className="mt-2 flex h-4 w-full overflow-hidden rounded-full">
                      {porStatus.filter((s) => s.count > 0).map((s) => (
                        <div key={s.value} title={`${s.label}: ${s.count}`} className="h-full transition-all"
            ...
            <Card className="p-6">
              ...
              {paralisadas.length === 0 && prazoVencido.length === 0 && execucaoLonga.length === 0 ? (
                <div className="flex items-center gap-2 rounded-xl bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700">✓ Nenhum alerta no momento</div>
              ) : (
                <div className="flex flex-col gap-3">
                  {paralisadas.length > 0 && (
                    <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3">
                      ...
                      <div className="mt-1.5 flex flex-col gap-1">
                        {paralisadas.slice(0,3).map((o) => <p key={o.id} className="text-xs text-red-600">{o.cliente} · desde {formatData(o.data_inicio)}</p>)}
                  ...
                  {prazoVencido.length > 0 && (
                    <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
                      ...
                      <div className="mt-1.5 flex flex-col gap-1">
                        {prazoVencido.slice(0,3).map((o) => <p key={o.id} className="text-xs text-amber-600">{o.cliente} · venceu {formatData(o.data_prazo!)} · {diasEntre(o.data_prazo!,HOJE)}d atrás</p>)}
                  ...
                  {execucaoLonga.length > 0 && (
                    <div className="rounded-xl border border-orange-200 bg-orange-50 px-4 py-3">
                      ...
                      <div className="mt-1.5 flex flex-col gap-1">
                        {execucaoLonga.slice(0,3).map((o) => <p key={o.id} className="text-xs text-orange-600">{o.cliente} · {diasEntre(o.data_inicio,HOJE)}d em execução</p>)}
          ...
          <Card className="p-6">
            ...
            {meses.length === 0 ? <p className="text-sm text-gray-400">Ajuste o período nos filtros.</p> : (
              <div className="flex flex-col gap-6">
                    ...
                    {meses.map((m) => (
                      <div key={m.key} className="flex flex-1 flex-col items-center gap-1">
                        <div className="flex w-full items-end justify-center gap-1" style={{ height:"80px" }}>
                          <div className="w-5 rounded-t-md bg-folk/70 transition-all duration-700" style={{ height:maxMes>0?`${(m.iniciadas/maxMes)*76}px`:"2px" }} title={`Iniciadas: ${m.iniciadas}`} />
                          <div className="w-5 rounded-t-md bg-emerald-500/80 transition-all duration-700" style={{ height:maxMes>0?`${(m.concluidas/maxMes)*76}px`:"2px" }} title={`Concluídas: ${m.concluidas}`} />
                        </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {meses.map((m, i) => (
                        <tr key={m.key} className={`border-b border-gray-50 last:border-0 ${i%2===0?"":"bg-gray-50/50"}`}>
                          ...
                          <td className="py-2.5 pr-4 text-center">
                            <span className={`font-bold ${m.saldo>0?"text-amber-600":m.saldo<0?"text-blue-600":"text-gray-400"}`}>{m.saldo>0?`+${m.saldo}`:m.saldo}</span>
                          </td>
                        </tr>
                      ))}
                      <tr className="border-t-2 border-gray-200 bg-gray-50 font-semibold">
                        ...
                        <td className="py-2.5 pr-4 text-center">{(() => { const s=meses.reduce((a,m)=>a+m.saldo,0); return <span className={`font-bold ${s>0?"text-amber-600":s<0?"text-blue-600":"text-gray-400"}`}>{s>0?`+${s}`:s}</span>; })()}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                    ...
                    {porTecnico.map((t, i) => (
                      <tr key={t.nome} className={`border-b border-gray-50 last:border-0 ${i%2===0?"":"bg-gray-50/50"}`}>
                        ...
                        <td className="py-3 pr-4 text-center">
                          {t.pctPrazo !== null ? (
                            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold ${ t.pctPrazo>=80?"bg-emerald-100 text-emerald-700":t.pctPrazo>=50?"bg-amber-100 text-amber-700":"bg-red-100 text-red-700" }`}>{t.pctPrazo}%</span>
                          ) : <span className="text-gray-400">—</span>}
                        </td>
                        <td className="py-3 pr-4 text-center">
                          {t.atrasoMedio > 0 ? <span className="font-semibold text-amber-600">{t.atrasoMedio}d</span> : <span className="text-gray-400">—</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
