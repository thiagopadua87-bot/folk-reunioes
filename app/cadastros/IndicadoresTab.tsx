...
export default function IndicadoresTab({ canEdit = true }: { canEdit?: boolean }) {
  ...
  const [filtros,    setFiltros]    = useState<FiltrosIndicadores>({ busca: "", ativo: null });

  const carregar = useCallback(async () => {
    setCarregando(true); setErro(null);
    try {
      setRegistros(await listarIndicadores({ busca: filtros.busca || undefined, ativo: filtros.ativo ?? undefined }));
    } catch (e) {
  ...
  function abrirEditar(r: Indicador) {
    setEditando(r);
    setForm({ nome: r.nome, telefone: r.telefone, email: r.email, tipo: r.tipo, ativo: r.ativo });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload: IndicadorPayload = {
        ...
        email: form.email.trim(), tipo: form.tipo, ativo: form.ativo,
      };
      if (editando) await editarIndicador(editando.id, payload);
      else          await criarIndicador(payload);
  ...
  async function toggleAtivo(r: Indicador) {
    try {
      await editarIndicador(r.id, {
        nome: r.nome, telefone: r.telefone, email: r.email, tipo: r.tipo, ativo: !r.ativo,
      });
      await carregar();
    } catch (e) {
      ...
      <div>
        <div className="mb-6 flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold text-gray-900">
            {editando ? "Editar indicador" : "Novo indicador"}
          </h2>
        </div>
        <Card className="p-6">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5 sm:col-span-2">
              ...
              <input type="text" value={form.nome} onChange={(e) => set("nome", e.target.value)} required placeholder="Nome completo ou razão social" className={INPUT} />
            </div>
            ...
            <div className="flex items-center gap-3 sm:col-span-2">
              <input type="checkbox" id="ativo-ind" checked={form.ativo} onChange={(e) => set("ativo", e.target.checked)} className="h-4 w-4 rounded border-gray-300 accent-folk" />
              <label htmlFor="ativo-ind" className="text-sm text-gray-700">Ativo</label>
            </div>
  ...
  return (
    <div>
      <Card className="mb-5 p-4">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-1.5">
            ...
            <input type="text" value={filtros.busca} onChange={(e) => setFiltros((f) => ({ ...f, busca: e.target.value }))} placeholder="Nome ou tipo..." className={INPUT} />
          </div>
          <div className="flex flex-col gap-1.5">
      ...
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-gray-500">{carregando ? "Carregando..." : `${registros.length} registro${registros.length !== 1 ? "s" : ""}`}</p>
        {canEdit && (
                ...
                <tr key={r.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="py-3.5 pr-6">
                    <div className="flex items-center gap-2">
                      {canEdit && (
                        <button onClick={() => abrirEditar(r)} className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-600 transition-colors hover:border-folk/30 hover:text-folk">
                          Editar
                        </button>
                      )}
