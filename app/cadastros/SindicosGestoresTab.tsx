...
import {
  listarSindicosGestores, criarSindicoGestor, editarSindicoGestor,
  TIPOS_SINDICO_GESTOR,
  type SindicoGestor, type SindicoGestorPayload, type TipoSindicoGestor,
  type FiltrosSindicosGestores,
} from "@/lib/cadastros";
...
export default function SindicosGestoresTab() {
  ...
  const [editando, setEditando] = useState<SindicoGestor | null>(null);
  const [form, setForm] = useState<FormState>(FORM_VAZIO);
  const [salvando, setSalvando] = useState(false);
  const [erroForm, setErroForm] = useState<string | null>(null);
  const [filtros, setFiltros] = useState<FiltrosSindicosGestores>({ busca: "", ativo: null });

  const carregar = useCallback(async () => {
    setCarregando(true); setErro(null);
    try {
      setRegistros(await listarSindicosGestores({ busca: filtros.busca || undefined, ativo: filtros.ativo ?? undefined }));
    } catch (e) {
  ...
  function abrirEditar(r: SindicoGestor) {
    setEditando(r);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload: SindicoGestorPayload = {
        nome: form.nome.trim(), telefone: form.telefone.trim(),
        email: form.email.trim(), tipo: form.tipo, ativo: form.ativo,
      };
      if (editando) await editarSindicoGestor(editando.id, payload);
      else          await criarSindicoGestor(payload);
  ...
  async function toggleAtivo(r: SindicoGestor) {
    try {
      await editarSindicoGestor(r.id, {
      ...
      <div>
        <div className="mb-6 flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold text-gray-900">
            {editando ? "Editar síndico/gestor" : "Novo síndico/gestor"}
          </h2>
        </div>
        <Card className="p-6">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select value={form.tipo} onChange={(e) => set("tipo", e.target.value as TipoSindicoGestor)} className={INPUT}>
            ...
            <div className="flex items-center gap-3 sm:col-span-2">
              ...
              />
              <label htmlFor="ativo-sg" className="text-sm text-gray-700">Ativo</label>
            </div>
    ...
    <div>
      <Card className="mb-5 p-4">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select
              value={filtros.ativo == null ? "" : String(filtros.ativo)}
              onChange={(e) => setFiltros((f) => ({ ...f, ativo: e.target.value === "" ? null : e.target.value === "true" }))}
              className={INPUT}
            >
      ...
      {!carregando && registros.length === 0 && !erro && (
        <div className="rounded-2xl border border-dashed border-gray-200 px-6 py-16 text-center text-sm text-gray-400">
          Nenhum síndico/gestor cadastrado.
        </div>
      )}
